...
function PubSub (topicID, opts) {
  ...
  this.peerID = opts.peerID || randombytes(ID_BASE).toString(ID_ENCODING)
...
PubSub.prototype._onConnectorDiscover = function (err, onlinePeerIDs) {
  ...
  onlinePeerIDs.sort(idComparator.bind(null, this.peerID))
  for (var i in onlinePeerIDs) {
    var pID = onlinePeerIDs[i]
    if (pID === this.peerID) continue

    var index = idCompare(pID, this.peerID) === -1 ? LEFT_INDEX : RIGHT_INDEX
...
PubSub.prototype._setupConnection = function (peerID, initiator) {
  ...
  var index = idCompare(peerID, this.peerID) === -1 ? LEFT_INDEX : RIGHT_INDEX
  var oldPeer = this._peers[index]
  this._peers[index] = peer
  if (oldPeer) oldPeer.destroy()
}

// Compares idA/B's distance to the targetID
function idComparator (targetID, idA, idB) {
  targetID = bigInteger(targetID, ID_BASE)
  idA = bigInteger(idA, ID_BASE)
  idB = bigInteger(idB, ID_BASE)

  // equivilent to Math.abs(targetID - idA) - Math.abs(targetID - idB)
  return targetID.minus(idA).abs().minus(targetID.minus(idB).abs())
}

function idCompare (idA, idB) {
  return bigInteger(idA, ID_BASE).compare(bigInteger(idB, ID_BASE))
}
